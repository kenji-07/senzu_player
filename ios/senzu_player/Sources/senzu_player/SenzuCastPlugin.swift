...
// google-cast-sdk нь CocoaPods-аар дэмжигддэг, SPM-д байхгүй.
// Тиймээс бүх GCK-хамааралтай кодыг #if canImport(GoogleCast) guard-аар хаав.
...
#else
// ─────────────────────────────────────────────────────────────────────────────
// SPM stub — GoogleCast байхгүй үед ашиглагдана (e.g. SPM build)
// ─────────────────────────────────────────────────────────────────────────────
public class SenzuCastPlugin: NSObject, FlutterStreamHandler {
    public static func register(
        with registrar: FlutterPluginRegistrar,
        ...
        let instance = SenzuCastPlugin()
        method.setMethodCallHandler { _, result in result(FlutterMethodNotImplemented) }
        event.setStreamHandler(instance)
    }
    public static func setupAfterInit() {}
    public func onListen(withArguments _: Any?, eventSink _: @escaping FlutterEventSink) -> FlutterError? { nil }
    public func onCancel(withArguments _: Any?) -> FlutterError? { nil }
}
#endif