...
@objc public class SenzuAVPlayerManager: NSObject {
    ...
    /**
     * Resolves any local-media string to a playable URL.
     *
     * Three formats are handled:
     *  1. "bookmark:<base64>" — persistent NSURLBookmarkData created at download
     *     time.  Immune to sandbox UUID changes and app updates.
     *  2. "file://..." — standard file URL; spaces/specials may need re-encoding.
     *  3. "/absolute/path" — raw path; rebuilt via URL(fileURLWithPath:) which
     *     handles spaces correctly.
     *
     * For paths inside Containers/Data/Application/<UUID> the UUID is patched
     * with the current NSHomeDirectory() UUID (legacy fallback for old records).
     */
    private func resolveLocalUrl(from urlString: String) -> URL? {
        // 1. Bookmark-based URL (recommended for HLS .movpkg)
        if urlString.hasPrefix("bookmark:") {
            if #available(iOS 10.0, *) {
                let resolved = SenzuDownloadManager.resolveBookmark(urlString)
                if let url = resolved {
                    print("[SenzuPlayer] Bookmark resolved to: \(url.path)")
                    return url
                }
            }
            print("[SenzuPlayer] Bookmark resolution failed, falling through to path fallback")
            return nil
        }

        // 2. file:// URL
        if urlString.hasPrefix("file://") {
            // Try URL(string:) first — works when the string is properly percent-encoded
            if let parsed = URL(string: urlString) {
                let resolvedPath = resolveSandboxPath(parsed.path)
                // If the path changed (UUID patched), rebuild; otherwise keep parsed URL
                // to preserve any trailing slash / directory indicators.
                if resolvedPath == parsed.path {
                    return parsed
                }
                return URL(fileURLWithPath: resolvedPath)
            }
            // Fallback: string had literal spaces/special chars — strip scheme and rebuild
            let rawPath = urlString.dropFirst("file://".count)
            let resolvedPath = resolveSandboxPath(String(rawPath))
            return URL(fileURLWithPath: resolvedPath)
        }

        // 3. Raw absolute path
        if urlString.hasPrefix("/") {
            let resolved = resolveSandboxPath(urlString)
    ...
    /// Patches the sandbox UUID in paths of the form
    ///   .../Containers/Data/Application/<OLD-UUID>/...
    /// to use the currently-active UUID from NSHomeDirectory().
    private func resolveSandboxPath(_ path: String) -> String {
        let home = NSHomeDirectory()
        let token = "Containers/Data/Application/"
        guard let pathTokenRange = path.range(of: token),
              home.range(of: token) != nil else {
            return path
        }

        let pathAfterToken = path[pathTokenRange.upperBound...]
        guard let slashIndex = pathAfterToken.firstIndex(of: "/") else { return path }
        let relativePath = pathAfterToken[slashIndex...]
    ...
    private func initialize(args: [String: Any]?, result: @escaping FlutterResult) {
        ...
        let isLocal = urlString.hasPrefix("file://")
            || urlString.hasPrefix("/")
            || urlString.hasPrefix("bookmark:")

        let url: URL?
        if isLocal {
            url = resolveLocalUrl(from: urlString)
        } else {
            url = URL(string: urlString)
        }

        guard let finalUrl = url else {
            result(FlutterError(code: "BAD_ARGS", message: "Cannot resolve URL: \(urlString.prefix(80))", details: nil))
            return
        }

        // Diagnostic: check local file existence before handing to AVPlayer
        if finalUrl.isFileURL {
            let exists = FileManager.default.fileExists(atPath: finalUrl.path)
            print("[SenzuPlayer] Local file exists=\(exists) path=\(finalUrl.path)")
            if !exists {
                result(FlutterError(
                    code: "INIT_ERROR",
                    message: "Local media file not found at: \(finalUrl.path)",
                    details: nil))
                return
            }
        }
